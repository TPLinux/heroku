...
scenes.buthow = function buthow(globals) {
    ...
    timeline.to(prevScene.node, 1, {
        scale: 0.04,
        svgOrigin: [vbWidth / 2, 280].join(' ')
    }, 'buthow').to(prevScene.node, 1, {
        x: vbWidth / 2 - 375,
        y: -140,
    }, 'buthow');
    ...
    var textbuble = scene.g(scene.rect({
        x: 0,
        y: 0,
        rx: 5,
        ry: 5,
    ...
    var startText;
    var prevP = startText = textbuble.g(hand, textbuble.text(-200, 0, "Who would you intrust the world's future to?").attr(objects.textStyle2)); // empty group for later swap
    TweenMax.set(prevP.node, {
        x: vbWidth / 2,
        y: 90
    });

    var playButton = textbuble.g(scene.rect({
        x: 0,
        y: 0,
        rx: 5,
        ry: 5,
    ...
    people.forEach(function(p) {
        ...
        p.add(p.puls);
        if (p.says) {
            p.says = textbuble.g(objects.make.multilineText(p.says, 50, objects.speachBubleTextStyle));
            TweenMax.set(p.says.node, {
                x: 100,
                y: 10
            });
        } else {
            text = textbuble.g(objects.make.multilineText(["I think I can help !"], 30, objects.speachBubleTextStyleYouth));
    ...
    timeline.fromTo(man.accessory.node, 0.1, {
        opacity: 1,
    }, {
        opacity: 0
    }, '_believe');
    timeline.to(man.node, 0.5, {
        scale: 0.5,
        y: '+=40',
        x: '+=10',
    ...
    text = objects.make.multilineText(["We believe that young people"], 50, objects.textStyle2);
    text.attr({
        'text-anchor': 'middle'
    });
    text = scene.g(text);
    ...
    timeline.add(tweens.slideInOut(prevScene, text, 250, Back.easeInOut, 0.5), '_believe-=0.3');


    timeline.add('_key', '_believe+=1.5');

    var keyhole = objects.make.keyhole(man);
    timeline.fromTo(keyhole.node, 1, {
        opacity: 0,
    }, {
        opacity: 1
    }, '_key');
    ...
    prevText = text;
    text = objects.make.multilineText(["Hold the key for a better future"], 50, objects.textStyle2);
    text.attr({
        'text-anchor': 'middle'
    });
    text = s.g(text);
    ...
    timeline.add(tweens.slideInOut(prevText, text, 250, Back.easeInOut, 0.5), '_key');
    ...
    text = objects.make.multilineText(["With their", "Passion & Entrepenurial spirit"], 50, objects.textStyle2);
    text = s.g(hightlight2, hightlight1, text);
    TweenMax.set(text.node, {
        x: vbWidth - 400
    });
    timeline.add(tweens.slideInOut(prevText, text, 250, Back.easeInOut, 0.5), '_spirit');
    var zoomX = scene.zoomX = (man.getBBox().cx + GreenProp.x(man.node) > 760) ? 760 : man.getBBox().cx + GreenProp.x(man.node);
    var zoomY = scene.zoomY = (man.getBBox().cy + GreenProp.y(man.node) - 100 > 590) ? 590 : man.getBBox().cy + GreenProp.y(man.node) - 100;
    timeline.to(scene.node, 1, {
        scale: 0.1,
        ease: Back.easeIn,
    ...
    var deathpillar = scene.death = scene.rect(-vbWidth * 3.6, -vbHeight * 7, 100, vbHeight * 8).attr({
        fill: '#fff'
    });
    ...
    text = objects.make.multilineText(["They can face tomorrow's issues"], 50, objects.textStyle2);
    text = s.g(text, underline);
    TweenMax.set(text.node, {
        x: vbWidth - 500
    });
    timeline.add(tweens.slideInOut(prevText, text, 250, Back.easeInOut, 0.5), '_spirit+=2');
    ...
    prevText = text;
    text = objects.make.multilineText(["With what they choose to do today"], 50, objects.textStyle2);
    text.attr({
        'text-anchor': 'middle'
    });
    ...
    timeline.add(tweens.slideInOut(prevText, text, 250, Back.easeInOut, 0.5), '_spirit+=' + (fallTween.duration() - 0.6));

    timeline.add(tweens.sequence().to(scene.node, 1.3, {
        scale: 0.2,
        svgOrigin: [zoomX, zoomY].join(' '),
        ease: Power1.easeIn
    }).to(scene.node, 0.6, {
