...
scenes.experience = function experience(globals) {
    ...
    timeline.set(glob.node, {
        fill: "#219EAB"
    }, 'experience');
    timeline.set(objects.background.node, {
        fill: "#20586A"
    }, 'experience');
    timeline.set(reflGr.node, {
            x: 442,
            scale: 1.5
        })
        .set(empowerGr.node, {
    ...
    var displayReflect = TweenMax.fromTo(reflGr.node, 0.3, {
        y: 800
    }, {
        y: 433
    });
    var displaySol = TweenMax.fromTo(solGr.node, 0.3, {
        y: 800
    }, {
        y: 433
    });
    var displayEmpower = TweenMax.fromTo(empowerGr.node, 0.3, {
        y: 800
    }, {
        y: 535
    });
    var displayWorld = TweenMax.fromTo(worldGr.node, 0.3, {
        y: 800
    }, {
        y: 535
    });
    ...
    var scalePowerUp1 = TweenMax.fromTo(outl1.node, 0.2, {
        scale: 0
    }, {
        scale: 0.32,
        ease: Back.easeOut
    });
    var scalePowerUp2 = TweenMax.fromTo(outl2.node, 0.2, {
        scale: 0
    }, {
        scale: 0.34,
        ease: Back.easeOut
    ...
    var scalePowerUp3 = TweenMax.fromTo(outl3.node, 0.2, {
        scale: 0
    }, {
        scale: 0.36,
        ease: Back.easeOut
    });
    var scalePowerUp4 = TweenMax.fromTo(outl4.node, 0.2, {
        scale: 0
    }, {
        scale: 0.38,
        ease: Back.easeOut
    ...
    ], 60, objects.textStyle4);
    text = s.g(text);
    TweenMax.set(text.node, {
        x: 100
    });
    timeline.add(tweens.slideInOut(prevText, text, 300, Back.easeInOut, 0.5), 'experience');
    ...
    timeline.add(clearColorAnim);
    timeline.add(tweens.sequence().add([manFlyUp, outlineFlyUp1, outlineFlyUp2, outlineFlyUp3, outlineFlyUp4]).duration(1.5));
    timeline.add([scaleDownPowerUp1, scaleDownPowerUp2, scaleDownPowerUp3, scaleDownPowerUp4, floatElem(man.node, 2, 20, 1)]);
    timeline.add([raiseRightArm, raiseLeftArm, openLeftLeg, openRightLeg, fillPowerUpBlue], "-=2.8");
    timeline.add(tweens.sequence().add([leftLegBack, rightLegBack, rightArmBack, leftArmBack]).duration(2), "-=2");
    timeline.add(manFly);
    ...
    prevText = text;
    text = objects.make.multilineText(["Through practical Experience"], 50, objects.textStyle4);
    text = s.g(text);
    TweenMax.set(text.node, {
        x: 100
    });
    timeline.add(tweens.slideInOut(prevText, text, 300, Back.easeInOut, 0.5), 'experience+=3');

    prevText = text;
    text = objects.make.multilineText(["In challenging enviroment"], 50, objects.textStyle4);
    text = s.g(text);
    TweenMax.set(text.node, {
        x: 100
    });
    timeline.add(tweens.slideInOut(prevText, text, 250, Back.easeInOut, 0.5), 'experience+=5.2');

    prevText = text;
    text = objects.make.multilineText(["And cross-cultural"], 50, objects.textStyle3);
    var text2 = objects.make.multilineText(["Exchange"], 50, objects.textStyle).attr({
        'font-weight': '700',
        'font-style':'italic',
        'text-anchor': 'begin'
    });
    ...
    timeline.add(tweens.slideInOut(prevText, text, 250, Back.easeInOut, 0.5), 'experience+=7.4');

    prevText = scene.lastText;
    text = objects.make.multilineText(["Striving to achieve", "Peace and fullfillment", "of Humankind's potential"], 50, objects.textStyle);
    text = scene.lastText = s.g(text);
    TweenMax.set(text.node, {x: vbWidth/2 - 340});
    timeline.add(tweens.slideInOut(prevText, text, 300, Back.easeInOut, 0.5), 'experience+=10.6');
