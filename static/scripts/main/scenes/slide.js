    ...
    var slide = function slide(globals) {
        ...
        var pathMask = objects.make.mask(path.clone().attr({
            stroke: '#fff',
            strokeWidth: 3,
            'stroke-dasharray': 'none'
        }), path);
        pathMask.tween = TweenMax.fromTo(pathMask.mask.node, 2, {
            drawSVG: '0%'
        }, {
            drawSVG: '100%',
        ...
        $(window).on('slide.reset', globals.rest);

        globals.trigger = function() {
            if (startTime <= globals.globalTimeline.time() + 0.1 && startTime >= globals.globalTimeline.time() - 0.1) {
        ...
        var background = scene.rect(-vbWidth / 2, -vbHeight / 2, vbWidth * 2, vbHeight * 2).attr({
        ...
        var text = objects.make.multilineText(elements[elementType].text, 60, objects.textStyle4);
        text.attr({
            'text-anchor': 'begin'
        });
        text = group.g(text);
        ...
        var mask = objects.make.mask(scene.circle(maskx, masky, 38).attr({
            fill: 'white'
        }), group);
        scene.add(mask);
        TweenMax.set(mask.mask.node, {
        ...
        var dragable = Draggable.create(mask.node, {
            ...
            onDrag: function() {
                ...
                if (drag.x > maxX) TweenMax.set(mask.node, {
                    x: maxX
                });
                if (drag.x < minX) TweenMax.set(mask.node, {
                    x: minX
                });
    ...
    var elements = {
        solution: {
            ...
            path: [
                ...
                ['C', vbWidth * 0.85, vbHeight * 0.8, vbWidth * 0.75, vbHeight * 0.8, vbWidth * 0.62, vbHeight * 0.72]
            ],
            x: 60,
            y: 0,
            color: '#955B9C',
        ...
        empower: {
            ...
            path: [
                ['M', vbWidth * 0.92, vbHeight * 0.65],
                ['C', vbWidth * 0.90, vbHeight * 0.7, vbWidth * 0.85, vbHeight * 0.7, vbWidth * 0.777, vbHeight * 0.6]
            ],
            x: 220,
            y: 0,
            color: '#E94F38',
            text: ['Communicates effectively', 'developing and empowering others', 'to achieve a bigger purpose.'],
            old_text: [
        ...
        reflect: {
            ...
            path: [
                ['M', vbWidth * 0.92, vbHeight * 0.65],
                ['C', vbWidth * 0.85, vbHeight * 0.8, vbWidth * 0.65, vbHeight * 0.8, vbWidth * 0.5, vbHeight * 0.62]
            ],
            x: -140,
            y: 0,
            color: '#F9CC00',
            text: ['Understands his personal values', ' and is clear on what he', 'is capable of doing.'],
            old_text: [
        ...
        world: {
            ...
            path: [
                ['M', vbWidth * 0.92, vbHeight * 0.65],
                ['C', vbWidth * 0.85, vbHeight * 0.8, vbWidth * 0.65, vbHeight * 0.8, vbWidth * 0.48, vbHeight * 0.66]
            ],
            x: -140,
            y: -40,
