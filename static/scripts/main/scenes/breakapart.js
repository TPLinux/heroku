...
scenes.breakapart = function breakapart(globals) {
    ...
    glob_text.tween = tweens.swipeOffStage(glob_text).set(glob_text.node, {
        opacity: 0
    ...
    var dragable = Draggable.create(glob_text.node, {
        ...
        onDrag: function() {
            ...
            if (drag.x > maxX) TweenMax.set(glob_text.node, {
                x: maxX
            });
            if (drag.x < minX) TweenMax.set(glob_text.node, {
                x: minX
            });
            ...
            if (drag.x >= 140) {
                $(window).trigger('timeline.play');
                drag.disable();
                if (glob_text.tween.paused()) { glob_text.tween.play(); }

                $(window).trigger('animation.started');
            }
        }
    });
    ...
    timeline.addCallback(function() {
        ...
        dragable[0].enable();
    }, 'start').set(glob_text.node, {
        opacity: 1
    }, 'start').addCallback(function() {
        if (!glob_text.swipping) {
            glob_text.tween.play(0);
        }
    }, 'start+=0.2').set(glob_text.node, {
        opacity: 0
    }, 'start+=2');

    // hiding pillar for drag effect
    timeline.set(deathpillar.node, {
        scale: 0,
        transformOrigin: '50% 0%'
    }, 'start').set(deathpillar.node, {
        scale: 1,
        transformOrigin: '50% 0%'
    }, 'start+=1');
    ...
    repeating.add([].map.call(floatingClouds.children(), function(c) {
        return c.tween;
    }), 0);
    repeating.repeat(-1);
    ...
    var seq = tweens.sequence().add(tweens.slip(man)).add(tweens.falling(man, 3)).add(tweens.crash(man), '-=0.25');
    timeline.add(seq.duration(timedown), 'slipfall');
    timeline.to(clouds.node, timedown - 0.5, {
        y: -vbHeight * 7,
        ease: Power0.easeIn
    }, 'slipfall');
    ...
    shadow.before(man);
    timeline.to(shadow.node, 0.3, {
        opacity: 0,
        x: '+=60'
    }, 'slipfall-=0.1').set(shadow.node, {
        scaleX: 1,
        x: '-=90',
        y: vbHeight * 7 + 140,
        opacity: 1,
        transformOrigin: '50% 50%'
    }, 'slipfall+=0.3').to(shadow.node, timedown - 0.5, {
        y: vbHeight - 595,
        ease: Expo.easeIn
    }, 'slipfall+=0.3').to(shadow.node, 0.5, {
        scaleX: 1.5,
        transformOrigin: '50% 50%',
    ...
    text = s.g(s.text(vbWidth - 300, 0, "It all started").attr(objects.textStyle));
    timeline.add(tweens.sequence().fromTo(text.node, 1, {
        y: vbHeight * 1.5
    }, {
        y: vbHeight / 2 - 150,
        ease: Power1.easeOut,
    ...
    var prevText = text;
    prevText = text;
    text = objects.make.multilineText(["After the Second World War"], 50, objects.textStyle);
    text = s.g(text);
    ...
    prevText = text;
    text = objects.make.multilineText(["As the", "Tension"], 50, objects.textStyle);
    text = s.g(text);
    TweenMax.set(text.node, {
        x: vbWidth - 300
    });
    ...
    for (var i = 1; i < 30; i++) {
        ...
        var tl = tweens.fallCrash(man, vbHeight - (425 + (i * 75))).duration(timedown);
        tl.timeScale(i);
        seq.add(tl, '-=' + (i / 600));
        men.add(man);
    }
    ...
    prevText = text;
    text = objects.make.multilineText(["Continued To", "Rise"], 50, objects.textStyle);
    text = s.g(text);
    TweenMax.set(text.node, {
        x: vbWidth - 300
    });
    timeline.add(tweens.slideInOut(prevText, text, 100, Back.easeInOut, 0.5), 'backup+=2');

    var changeColorRed = TweenMax.fromTo(objects.background.node, 3, {
        fill: "#3B363F"
    }, {
        fill: "#4F171F"
    });
    ...
    timeline.to(clouds.node, timedown * 1.4, {
        y: 0,
        ease: Back.easeIn
    }, 'backup');

    
    timeline.to(men.node, timedown * 0.9, {
        // scale: 0.2,
        y: (vbHeight * 1),
        // svgOrigin: vbWidth/2 + ' ' + (vbHeight*3),
        ease: Power1.easeOut
    }, 'backup+=2').to(men.node, 1.5, {
        scale: 0.2,
        ease: Power1.easeOut,
    ...
    timeline.to(deathpillar.node, 1.5, {
        scale: 0.2,
        transformOrigin: '57% 0%'
    }, 'map-=1');

    timeline.from(deathpillar.node, 0.8, {
        fill: 'rgba(255,255,255,0)',
        opacity: 0,
        // strokeDasharray: '0'
    }, 'map-=0.3').to(men.node, 0.5, {
        opacity: 0
    }, 'map');


    prevText = text;
    text = objects.make.multilineText(["Between", "Nations"], 50, objects.textStyle);
    text = s.g(text);
    TweenMax.set(text.node, {
        x: vbWidth - 300
    });
    timeline.add(tweens.slideInOut(prevText, text, 100, Back.easeInOut, 0.5), 'map-=1');
    ...
    timeline.to(map.node, 1, {
        opacity: 1
    }, 'map-=3');
    timeline.to(floatingClouds.node, 1, {
        scale: 0.5,
        svgOrigin: [x, y].join(' ')
    }, 'map-=3');
    ...
    TweenMax.set(mapclouds.node, {
        y: '-=' + 50
    }, 'map');

    timeline.to(mapclouds.node, 0.5, {
        y: '+=100'
    }, 'map');

    var mapcloudsmen = scene.g(deathpillar, men, clouds);

    timeline.to(mapcloudsmen.node, 1, {
        x: '-=200'
    }, 'map-=1');
    ...
    timeline.add(tweens.sequence().to(mapcloudsmen.node, 1, {
        scale: '+=0.2',
        svgOrigin: [x, y].join(' '),
        ease: Elastic.easeIn
    }).add(map.explossionTween).add(tweens.scatter(floatingClouds), 1), 'breakappart-=0.3');
    ...
    prevText = text;
    text = objects.make.multilineText(["Breaking Them", "Apart"], 50, objects.textStyle);
    text = s.g(text);
    TweenMax.set(text.node, {
        x: vbWidth - 300
    });
    timeline.add(tweens.slideInOut(prevText, text, 100, Back.easeInOut, 0.5), 'breakappart+=0.3');

    var blinkColorRed = TweenMax.fromTo(objects.background.node, 0.2, {
        fill: "#3B363F"
    }, {
        fill: "#4F171F",
        repeat: 2
