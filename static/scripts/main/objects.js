(function() {
    ...
    var load = function(svg) {
        ...
        backstage.add(trees);
        objects.ready = true;
        $(window).trigger('objects.loaded', objects);
    };
    ...
    objects.make = {
        ...
        glob: function makeGlob(opt) {
            ...
            var semicircle = backstage.circle(opt.cx || centerStage, opt.cy || vbHeight, semicriclerad);
            semicircle.attr({
        ...
        butHowText: function() {
            ...
            subtexts[1].attr({
                x: subtexts[1].getBBox().width - 3200 > 0 ? subtexts[1].getBBox().width - 3200 : 5000,
                'font-style': 'italic'
            });
    ...
    var background = objects.background = s.rect(-vbWidth, -vbHeight, vbWidth * 3, vbHeight * 3);
    background.attr({
        fill: s.gradient('L(0,0, ' + vbHeight * 3 + ', ' + vbHeight * 3 + ')#3f3a42-#27212b')
    });
    ...
    objects.make.stick = function() {
        ...
        stick.data('morphs', [stick.attr('d'), 'm 0,0 c 97.6826,57.84069 287.8299,64.59687 470,0']);
        return stick;
    };
    ...
    objects.make.oldman = function() {
        ...
        obj.offset = -280;
        obj.says = ["I have great wisdom to share, alase...", "I'm sorry, I don't have the energy these days"];
        obj.shadowx = 80;
        obj.shadowy = 370;
        return obj;
    };
    objects.make.businessman = function() {
        ...
        obj.offset = 305;
        obj.says = ["What's that? Sorry, I have a lot of work to do", "Not enough time"];
        obj.shadowx = 110;
        obj.shadowy = 450;
        return obj;
    };
    ...
    objects.make.kid = function() {
        ...
        obj.says = ["You realise I don't even know what a passport is...", " Hey wanna play Ball!."];
        obj.shadowx = 65;
        obj.shadowy = 210;
        return obj;
    };
    objects.make.mom = function() {
        ...
        obj.offset = obj.getBBox().y2;
        obj.says = ["I have to take care of my child...", "He may grow up to be a world leader!"];
        obj.shadowx = 80;
        obj.shadowy = 420;
        return obj;
    };
    objects.make.dad = function() {
        ...
        obj.offset = obj.getBBox().y2;
        obj.says = ["Who do you think I am!", "I need to make a living for my family"];
        obj.shadowx = 100;
        obj.shadowy = 430;
        return obj;
    };
    ...
    objects.makeSwipeText = objects.make.swipText = function makeSwipeText() {
        var swiptext1 = backstage.text(centerStage, vbHeight - 200, "Swipe to Learn").attr({
            fill: "white",
            'text-anchor': 'middle',
            "font-size": "45px",
            "font-family": "'Lato', Arial, sans-serif",
        ...
        var swiptext2 = backstage.text(centerStage, vbHeight - 150, "About AIESEC").attr({
            fill: "white",
            'text-anchor': 'middle',
            "font-size": "50px",
            "font-family": "'Lato', Arial, sans-serif",
        ...
        var underline = makeLine(swipX, vbHeight - 140, centerStage + linewidth, vbHeight - 140);

        var swipeText = backstage.g(swiptext1, swiptext2, upperline, underline, leftArrow, rightArrow);
    ...
    objects.make.map = function makeMap(opt) {
        ...
        map.selectAll('.location').forEach(function(l) {
            ...
            location.puls = puls;
            var bb = p.getBBox();
            var interactionArea = location.group.ellipse(cx, cy - bb.height * 0.035 * 0.5, bb.width * 0.035 * 1.2, bb.height * 0.035 * 1.2).attr({
                fill: 'rgba(0,0,0,0)',
                'class': 'pointer',
                cursor: 'pointer'
            });
            ...
            var toggleAction = opt.toggleAction || function toggleAction() {
        ...
        map.tween = tweens.sequence({
            onComplete: function() {
                if (!map.connected) {
                    // $(window).trigger('scenes.youngpeople.timeline.add', [map.tween, 'youngpeople']);
    ...
    objects.makeReflectman = objects.make.reflectman = function makeReflectman(opt) {
        opt = opt || {};
        ...
        manreflect.otherform.parts = manreflect.otherform.node.querySelectorAll('path');
    ...
    objects.makeRecallCircle = function makeRecallCircle(x, y, color, stroke) {
        ...
        var mask = objects.make.mask(backstage.circle(0, 0, 120).attr({
            fill: 'white'
        }));
    ...
    objects.make.saveWorldSubScene = function makeSaveWorldSubScene(x, y, path, scene) {
        var group = backstage.g();
        var saveMask2 = objects.make.mask(objects.backstage.circle(0, 0, vbWidth / 2).attr({
            fill: 'white'
        }), scene.scene, {
            x: -vbWidth / 2 - 300,
            y: -vbHeight / 2
